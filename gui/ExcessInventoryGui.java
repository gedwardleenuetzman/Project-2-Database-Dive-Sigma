...
/**
 * ExcessInvetoryGUI to extend the JFrame within the GUI
 */
public class ExcessInventoryGui extends JFrame {
	/**
	 * 
	 * @param gui       gui to go back to when x is pressed
	 * @param startDate start date to get the excess report from
	 * @param endDate   end date when to close the time period in which the excess
	 *                  report is created
	 */
	public ExcessInventoryGui(ExcessReportGui gui, String startDate, String endDate) {
		/**
		 * Provides functionality so that an excess report can be created
		 * uses the ability to get a start and end date to which the report will
		 * pull data from the sql tables to see which item has used less than ten
		 * percent
		 * of the invetory
		 */
		try {
			...
			while (orderResult.next()) {
				ResultSet order_products = statement.executeQuery(
						"SELECT * FROM order_products WHERE order_id = " + orderResult.getInt("id") + ";");
				Statement statement2 = conn.createStatement();

				while (order_products.next()) {
					ResultSet product = statement2.executeQuery(
							"SELECT * FROM products_cfa WHERE id = " + order_products.getInt("prod_id") + " LIMIT 1;");
					product.next();
					...
					if (product.getString("name") != null) {
						...
						if (product_quantity.get(name) != null) {
							...
							totals.put(name,
									totals.get(name) + order_products.getInt("quantity") * product.getFloat("price"));
						} else {
					...
					ResultSet ingredient = statement3.executeQuery(
							"SELECT * FROM product_ingredients WHERE prod_id = " + product.getInt("id") + ";");

					while (ingredient.next()) {
						ResultSet ingredient_name = statement4.executeQuery(
								"SELECT * FROM ingredients WHERE id = " + ingredient.getInt("prod_id") + " LIMIT 1;");
						ingredient_name.next();
						String ingredient_String = ingredient_name.getString("name");

						if (ingredient_sold.get(ingredient_String) != null) {
							ingredient_sold.put(ingredient_String,
									ingredient_sold.get(ingredient_String) + ingredient.getInt("quantity"));
