...
/**
 * Will return a report that shows the items that need to be restocked
 */
public class RestockReportGui extends JFrame {
	/**
	 * 
	 * @param gui goes to the previous gui (managerGUI)
	 */
			...
			for (String id : inv.keySet()) {
				...
				if (invCount < restockCount) {
					JLabel label = new JLabel(
							id2name.get(id) + " needs to be restocked (" + invCount + " / " + restockCount + ")");
